...
static void liteeth_setup_slots(struct udevice *dev, struct liteeth *priv)
{
	ofnode np = dev_ofnode(dev);
	int err;

	err = ofnode_read_u32(np, "litex,rx-slots", &priv->num_rx_slots);
	if (err) {
		dev_dbg(dev, "unable to get litex,rx-slots, using 2\n");
		priv->num_rx_slots = 2;
	}

	err = ofnode_read_u32(np, "litex,tx-slots", &priv->num_tx_slots);
	if (err) {
		dev_dbg(dev, "unable to get litex,tx-slots, using 2\n");
		priv->num_tx_slots = 2;
	}

	err = ofnode_read_u32(np, "litex,slot-size", &priv->slot_size);
	if (err) {
		dev_dbg(dev, "unable to get litex,slot-size, using 0x800\n");
